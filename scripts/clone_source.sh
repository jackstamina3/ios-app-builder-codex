...
if [[ -f "$SOURCE_DIR/.gitmodules" ]]; then
  ...
  done < <(git -C "$SOURCE_DIR" config -f .gitmodules --get-regexp '^submodule\..*\.url$' | awk '{print $2}')
  git -C "$SOURCE_DIR" -c protocol.file.allow=never -c protocol.ssh.allow=never submodule update --init --recursive --depth=1
fi
